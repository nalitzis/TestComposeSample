...
import com.example.jetnews.ui.mytest.MyTestScreen
    ...
    Crossfade(JetnewsStatus.currentScreen) { screen ->
        Surface(color = (+MaterialTheme.colors()).background) {
            when (screen) {
                ...
                is Screen.MyTestScreen -> MyTestScreen { openDrawer() }
            }
        }
    }
}
...
private fun AppDrawer(
    ...
    Column(modifier = Expanded) {
        ...
        DrawerButton(
            icon = R.drawable.ic_interests,
            label = "Test",
            isSelected = currentScreen == Screen.MyTestScreen
        ) {
            navigateTo(Screen.MyTestScreen)
            closeDrawer()
        }
    }
}
